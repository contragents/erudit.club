...
";
    ...
    {
        $res = [];
        $res[] = 'Карточка спонсора проекта выдается навсегда и отображается в меню Игроки, а также в Статистике игроков (вкладка Награды)'
            . VH::br(2)
            . self::patronCard();
        ...
        $res[] = 'Наш игровой проект существует благодаря поддержке спонсоров. Вы можете присоединиться к команде патронов, приобретя любое количество игровых монет (меню Профиль->Кошелек)'
            . VH::br(2)
            . 'За вклад в проект выдается карточка спонсора. Уровень карточки растет по мере роста вклада'
            . VH::br(2)
            . self::patronCard();

        $res[] = 'Стать партнером можно, пригласив рефералов по личной ссылке (кнопка "Добавить друга" или меню Профиль->Рефералы)'
            . VH::br(2)
            . 'За вклад в проект выдается карточка спонсора. Уровень карточки растет по мере роста вклада'
            . VH::br(2)
            . self::patronCard();
