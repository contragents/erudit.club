...
class Hints
{
    ...
    const EXCEPTIONS = [
        'Android' => [ // Подсказки, зависящие от приложения Андроид
            'share' => 'isAndroidApp',
            'newUpdateAndroidAppHint' => 'IsNotAndroidApp',
    ...
    private static function tgWordCheckHint(): string
    {
        return '<strong>Подбор слов</strong> по словарю Эрудита доступен в <a target="_blank" href="https://t.me/erudit_club_bot">Телеграм-боте</a>';
    }

    private static function newUpdateAndroidAppHint(): string
    {
        return '<strong>Внимание!</strong><br /> Вышло обновление Игры. Для применения изменений, пожалуйста, обновите кеш приложения:<br />Нажать шестерёнку справа вверху<br />Выбрать пункт Приложения<br />В списке приложений найти Эрудит, нажать на него<br />Выбрать пункт меню Память<br />Нажать Очистить кэш справа внизу. Только кэш, НЕ данные';
    }

    private static function newUpdateHint(): string
    {
...
";
    ...
    private static function yandexScoreLink(): string
    {
        return 'Оцените Игру по <a href="' . self::YANDEX_RATING_URL . '" target="_blank">ссылке</a> - откроется в новом окне';
    }

    private static function supportHint(): string
    {
        if (self::isYandexApp()) {
            return '';
        }
    ...
    {
        return VH::img(
            ['src' => Config::$config['domain'] . "/images/purple_compact_day_RU.png", 'height' => "{$height}px"]
        );
    }

    private static function donationHint(): string
    {
        $res1 = 'Карточка спонсора проекта выдается навсегда и отображается в меню Игроки, а также в Статистике игроков (вкладка Награды)'
            . VH::br(2)
            . self::patronCard();

        $res2 = T::S(
                'Вы можете стать спонсором нашего игрового проекта, поддержав его донатом. Зайдите в меню Профиль->Кошелек, оплатите любое количество монет {{sudoku_icon_5}} и получите карточку спонсора - '
            )
            . VH::br(2)
            . self::patronCard()
            . VH::br(2)
            . 'Уровень карточки и ежедневный доход монет растут по мере роста вашей поддержки - от <strong>Партнера</strong> (1 монета в день) до <strong>Мецената</strong> (1000 монет каждый день)';

        $res3 = 'Наш игровой проект существует благодаря поддержке спонсоров. Вы можете присоединиться к команде патронов, приобретя любое количество игровых монет (меню Профиль->Кошелек)'
            . VH::br()
            // todo CLUB-468 CLUB-471 доделать рефералов
            // . 'Также стать партнером можно, пригласив рефералов по личной ссылке (кнопка "Добавить друга" или меню Профиль->Рефералы)'
            . VH::br()
            . 'За вклад в проект выдается карточка спонсора. Уровень карточки растет по мере роста вклада'
            . VH::br(2)
            . self::patronCard();

        return [$res1, $res2, $res3][array_rand([$res1, $res2, $res3])];
    }
...
|fone|hiptop|mini|mobi|palm|phone|pie|tablet|up\.browser|up\.link|webos|wos)/i"
    ...
    public static function isNotYandexApp(): bool
    {
        return !self::isYandexApp();
    }
