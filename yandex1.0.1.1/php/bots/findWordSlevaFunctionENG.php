...
function findWordSleva($x, $y, &$desk, &$fishki, array &$slovaPlayed, $orientation = 'all')
{
    ...
    while ($desk[$x + $step][$y][0]) {
        $lastLetter .= \Lang\Eng::$bukvy[($desk[$x + $step][$y][1] < 999 ? $desk[$x + $step][$y][1] : $desk[$x + $step][$y][1] - 999 - 1)][0];
        $maxWordLen++;
        $step++;
    }
        ...
        foreach ($res as $row)
            if (!isset($slovaPlayed[$row['slovo'] = mb_strtolower($row['slovo'], 'UTF-8')])
                ...
                for ($k = 0; $k < $slovoNach; $k++) {
                    $cells[$x - $k][$y][0] = true;
                    $cells[$x - $k][$y][1] = \Lang\Eng::getLetterCode($letter = mb_substr($row['slovo'], $slovoNach - 1 - $k, 1, 'UTF-8'));
                    if (isset($lettersZvezd[$letter]))
                        $cells[$x - $k][$y][2] = $lettersZvezd[$letter];//Указали на занятую звездочку
                }
                ...
                for ($k = $slovoNach + $lastLetterLen; $k < mb_strlen($row['slovo'], 'UTF-8'); $k++) {
                    ...
                    $cells[$x + $k - $slovoNach + 1 + $delta][$y][1] = \Lang\Eng::getLetterCode($letter = mb_substr($row['slovo'], $k, 1, 'UTF-8'));
                    if (isset($lettersZvezd[$letter]))
                        $cells[$x + $k - $slovoNach + 1 + $delta][$y][2] = $lettersZvezd[$letter];//Указали на занятую звездочку
                }
                printr($cells);
                //sleep(2);
                $desk = $cells;
