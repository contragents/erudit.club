        ...
        action: function(data) {
            ...
            if ('players'in data) {
                ...
                const ratingRadio = (props) => {
                    const {title='', text='', inputValue=0, inputId='0', isChecked=false, isDisabled=false, name='from_rating', extraClass='', extraInputAttrString='', } = props;
                ...
                const getBidList = (bidValues=[], data={}) => {
                    ...
                    bidValues.forEach( (bidValue) => {
                        if ('coin_players'in data && bidValue in data['coin_players'] && data['coin_players'][bidValue] >= 0) {
                            let isChecked = false;
                            if ('prefs'in data && data['prefs'] !== false && 'bid'in data['prefs'] && data['prefs']['bid'] == bidValue) {
                                isChecked = true;
                            }
                            resultHtml += ratingRadio({
                                title: data['coin_players'][bidValue] + ' in game',
                                text: `<img src="/images/coin.png" alt="SUDOKU coin image" width="15%"> ${bidValue}`,
                                inputValue: bidValue,
                ...
                onlinePlayers += `<div class="box-title-wrap">
												<span>Choose your MAX bet</span>
											</div>`;

                const bids = Object.keys(data.coin_players).filter( (item) => !isNaN(Number(item)) && data.coin_players[item] >= 0);
                ...
                onlinePlayers += getBidList(bids.slice(0, bids.length / 2), data);
                onlinePlayers += `	</div>`;
                // end col

                onlinePlayers += `	<div class="form-check">`;
                if (bids.slice(bids.length / 2).length > 0) {
                    onlinePlayers += getBidList(bids.slice(bids.length / 2), data);
                }
                onlinePlayers += `	</div>`;
                // end col
            ...
            let radioButtons = `<div style="display:none;" class="form-check form-check-inline"><input class="form-check-input" type="radio" id="twoonly" name="players_count" value="2" checked> <label class="form-check-label" for="twoonly">
                Just two players                </label></div>`;
            radioButtons += `<div style="display:none;" class="form-check form-check-inline"><input class="form-check-input" type="radio" id="twomore" name="players_count" value="4"> <label class="form-check-label" for="twomore">
                Up to four players                </label></div>`;
                        ...
                        oferta: {
                            ...
                            callback: function() {
                                async function getOfertaModal() {
                                    return fetch('/oferta.html').then( (response) => response.text());
                                }
                                ;console.log('!!!!!!!!!!!');
                    ...
                    && {
						...
						`;
                                        ...
                                        function getProfileModal(profileData) {
                                            return fetch('/profile-modal-tpl_1.html' + '?ver=' + Math.floor(Date.now())).then( (response) => response.text()).then( (template) => {
...
function commonCallback(data) {
        ...
        if (canOpenDialog) {
            if (gameState == 'initGame' || gameState == 'initRatingGame' || gameState == 'initCoinGame') {
                ...
                setTimeout(function() {
                    bootbox.hideAll();
                    var message = '';
                    var cancelLabel = 'Close in 5 seconds';
    ...
    if ('bid'in data && gameBid === false) {
        ...
        preloaderObject.load.svg('bankBlockOtjat', `/img/otjat/${players.bankBlock.filename}${gameBankString}.svg`, {
            ...
            'height': 'height'in players.bankBlock ? players.bankBlock.height : buttonHeight,
        });
...
function RobokassaPaymentGlobal(actionType) {
    ...
    if (!amountToBuy || amountToBuy < 0) {
        amountToBuy = 0;
        button.html('Replenish');
        calcPriceElement.html('calc_price');
    }
...
function chatButtonFunction() {
    ...
    if (ochki_arr.length > 1) {
        radioButtons += '<div style="font-size: 70%;" class="form-check form-check-inline"><input class="form-check-input" type="radio" id="chatall" name="chatTo" value="all" checked> <label class="form-check-label" for="chatall">For everyone</label></div>';
        isSelectedPlaced = true;
    }

    for (k in ochki_arr) {
        if (k != myUserNum) {
            radioButtons += '<div style="font-size: 70%;" class="form-check form-check-inline"><input class="form-check-input" type="radio" id="to_' + (k == 0 ? '0' : k) + '" name="chatTo" value="' + (k == 0 ? '0' : k) + '" ' + (isSelectedPlaced ? '' : ' checked ') + '> <label class="form-check-label" for="to_' + (k == 0 ? '0' : k) + '">To Player' + (parseInt(k, 10) + 1) + '</label></div>';
            isSelectedPlaced = true;
        }
    }

    radioButtons += '<div style="font-size: 70%;" class="form-check form-check-inline"><input class="form-check-input" type="radio" id="to_words" name="chatTo" value="words" ' + (isSelectedPlaced ? '' : ' checked ') + '> <label class="form-check-label" for="to_words">Word matching</label></div>';
...
function PlayersPage(json) {
    function Card({event_type, event_period, record_type_text, event_type_text, points_text, reward, income, date_achieved, }=props) {
        const types = {
            day: 'stone_card',
            week: 'bronze_card',
            month: 'silver_card',
    ...
    const CardCompact = (props) => {
        const {event_value, event_period, record_type_text, event_type_text, reward, record_type, event_type, } = props;
    ...
    const PlayerBox = (props) => {
        const {common_id, you, nickname, avatar_url, stats_url, is_balance_hidden, balance, rating, rating_position, games_played, index, achieves=[], top_bage_url='', } = props;
        ...
        if (you) {
            ...
            boxLabel = 'Player' + (index + 1);
        }

        let awards;
        let cards = achieves.map( (item) => {
		...
		<div class="box box-player">
			...
			<div class="d-flex mb-2">
				...
				<button class="btn btn-sm ml-auto js-modal-stats" data-user-id="${common_id}">Stats</button>
			</div>
			<div class="d-flex" style="margin-bottom: 5px;">
				...
				<div class="info-col">
					<ul>
						...
						<li>
							<div class="label">Ranking number</div>
							<div class="pill">${rating_position}</div>
						</li>
						<li>
							<div class="label d-flex align-items-center">Balance<i class="icon icon-coin ml-2"></i></div>
            ...
            if (props) {
                ...
                const modalHtml = `
							<div class="box d-flex align-items-center p-2 mb-2">
								<div class="box-heading text-center mx-auto fs-4">Reward</div>
							</div>
    ...
    function init() {
        ...
        const playerBoxes = json.map( (element, i) => PlayerBox({
            ...element,
            index: i
        })).join('');

        const html = `<div><div class="box d-flex align-items-center p-2 mb-2">
						<div class="box-heading text-center mx-auto fs-4">Players</div>
					</div>
					${playerBoxes}</div>`;
