                        ...
                        callback: function () {
                            setTimeout(function () {
                                fetchGlobal(CABINET_SCRIPT, '', 12).then((dataCabinet) => {
                                    if (dataCabinet == '') var responseText = 'Error';
                                        ...
                                        return fetch('/profile-modal-tpl.html'+ '?ver=' + Math.floor(Date.now()))
                                            ...
                                            .then((template) => {
                                                ...
                                                let message = template
                                                    ...
                                                    .replaceAll('{{Referrals}}', 'Referrals')
                                                    .replaceAll('{{Player ID}}', 'Player ID')
                                                    .replaceAll('{{Save}}', 'Save')
                                                    .replaceAll('{{Input new nickname}}', 'Input new nickname')
                                                    .replaceAll('{{Your rank}}', 'Your rank')
                                                    .replaceAll('{{Ranking number}}', 'Ranking number')
                                                    .replaceAll('{{Balance}}', 'Balance')
                                                    .replaceAll('{{Rating by coins}}', 'Rating by coins')
                                                    .replaceAll('{{Link}}', 'Link') // Привязать
                    ...
                    beginGame: {
                        ...
                        callback: function () {
                            ...
                            fetchGlobal(
                                ...
                                if (data == '') var responseText = 'Error';
                                else {
                                    commonCallback(data);
                                }
                            });
...
function savePlayerAvatar(url, commonIdParam) {
    ...
    let URL = useLocalStorage
        ? (
            '/scrabble1.0.1.6/php/yowser/index.php'
            + '?cooki='
...
async function fetchGlobalNominal(script, param_name, param_data) {
    const response = await fetch('/scrabble1.0.1.6/php/'
        + script
        + '?'
        + commonParams(),
        {
...
async function fetchGlobalYowser(script, param_name, param_data) {
    const response = await fetch('/scrabble1.0.1.6/php/yowser/index.php'
        + '?cooki='
        + localStorage.erudit_user_session_ID
        + '&script='
        + script
