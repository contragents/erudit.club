...
class Game
{
    ...
    private function getPlayerName(array $user)
    {
        ...
        $res = DB::queryValue("SELECT name FROM player_names 
            ...
            foreach(str_split($sintName) as $index => $lowByte) {
                ...
                if ($this->gameStatus['lngClass']::$bukvy[$letterNumber][2] * $letterNumber >= 30) {
...
LIMIT 40";
    ...
    public function submitTurn()
    {
        ...
        try {
            ...
            $this->addToLog('зарабатывает ' . $ochkiZaHod . ' за ход #' . $this->gameStatus['turnNumber'] . $this->logSlov($new_fishki['words']), $this->numUser);
    ...
    {
        return $this->p->redis->hget("erudit.inviteplayers_waiters", $this->User);
    }
            ...
            if ($this->gameStatus['users'][$this->gameStatus['activeUser']]['lostTurns'] >= 3) {
                ...
                if (isset($result['winner'])) {
                    if ($result['winner'] == $this->User) {
                        ...
                        return $this->makeResponse(['gameState' => 'gameResults', 'comments' => "<strong style=\\", 'desk' => unserialize($desk)]);
                    }
    ...
    public function newGame()
    {
        ...
        */
        ...
        $this->p->redis->hdel('erudit.rating_waiters', $this->User);
        $this->p->redis->hdel('erudit.ratingEN_waiters', $this->User);
        //Удаляем из списка ждунов на рейтинг

        $this->p->redis->hdel("erudit.2players_waiters", $this->User);
        $this->p->redis->hdel("erudit.2ENplayers_waiters", $this->User);
        $this->p->redis->hdel("erudit.4players_waiters", $this->User);
        $this->p->redis->hdel("erudit.4ENplayers_waiters", $this->User);
        //Удаляем из 2&4 очередей
        $this->p->redis->hdel("erudit.inviteplayers_waiters", $this->User);
        $this->p->redis->hdel("erudit.inviteENplayers_waiters", $this->User);

        $this->p->redis->del('erudit.get_game_' . $this->User);
        //Удалили указатель на текущую игру для пользователя
    ...
    private function processInvites(&$arr)
    {
        ...
        if ($this->gameStatus['invite'] == 'newGameStarting') {
            ...
            (new Queue($this->User, $this, $this->p, []))->storePlayersToQueue($this->User, 'invite');
