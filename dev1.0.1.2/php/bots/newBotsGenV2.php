...
class BotGen
{
    ...
    public function  __construct() {
        ...
        while  ( (date('U') - $start_script_time) < $script_work_time) {
            ...
            if( $llen=$red->redis->llen('erudit.games_waiting_v2') )
                for($i=0;$i<$llen;$i++)
                    if ($waiting_game = $red->redis->lpop('erudit.games_waiting_v2')) {
                        ...
                        print_r($waiting_game); print  $config['gameWaitLimit'];//sleep(4);
                            ...
                            if ( (count($waiting_game) <= 3) && (count($waiting_game)>=1) ) {
                                    ...
                                    if (count($waiting_game))
                                        $red->redis->rpush('erudit.games_waiting_v2',serialize($waiting_game));
                                    ...
                                    if ($noBots)
                                        if ( (date('U') - $minTime + 5) > $config['gameWaitLimit'] )
                                            ...
                                            if ($newBot = genNewBot()) {
                                                ...
                                                print ' '.$minTime.' - не удалось создать бота genNewBot'."\n";
                                            else
                                                print ' '.date('U').'-'.$minTime.' - не подошло время создавать бота'."\n";
                                         
                                        
                            }
                            /*
                            else
                                    $red->redis->lpush('erudit.games_waiting_v2',serialize($waiting_game));
