...
class BotGenV3ENG
{
                ...
                if ($this->timeToMake4Game()) {
                    if ($this->noBots4Waiting()) {
                        if ($newBot = $this->genNewBot()) {
                            ...
                            $this->p->redis->rpush('erudit.botEN_games', $newBot);
    ...
    private function startGame($botName)
    {
        ...
        ob_end_clean();
        //Пинганули сервер, что есть новый игрок
        $this->p->redis->rpush('erudit.botEN_games', $botName);
        //Сохранили нового бота в список игроков
    }
    ...
    private function storeTo2Players($User)
    {
        ...
        if (!$this->p->redis->hget('erudit.2ENplayers_waiters', $User)) {
            $this->p->redis->hset(
    ...
    {
        if (!$this->p->redis->hget('erudit.4ENplayers_waiters', $User)) {
            if (isset($this->POST['ochki_num'])) {
                $options = $this->POST;
            } else {
                $options = false;
    ...
    {
        $allPlayers2Waiting = $this->p->redis->hgetall('erudit.2ENplayers_waiters');
    ...
    {
        $waitingPlayers = $this->p->redis->hgetall('erudit.2ENplayers_waiters');
        $maxTimeWaiting = 0;
    ...
    {
        $cnt = $this->p->redis->hlen('erudit.2ENplayers_waiters');
        if ($cnt >= 1) {
            return true;
        }
        return false;
    ...
    {
        $allPlayers4Waiting = $this->p->redis->hgetall('erudit.4ENplayers_waiters');
    ...
    {
        $waitingPlayers = $this->p->redis->hgetall('erudit.4ENplayers_waiters');
        $maxTimeWaiting = 0;
        foreach ($waitingPlayers as $player => $data) {
            $data = unserialize($data);
            if ((date('U') - $data['time']) > $maxTimeWaiting) {
    ...
    {
        $cnt = $this->p->redis->hlen('erudit.4ENplayers_waiters');
        print $cnt;
        if (($cnt >= 1) && ($cnt < 4)) {
            return true;
        }
