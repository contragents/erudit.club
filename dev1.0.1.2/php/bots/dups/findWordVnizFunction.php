...
function findWordVniz($x, $y, &$desk, &$fishki, array &$slovaPlayed, $orientation = 'all') {
    ...
    $maxWordLen=$maxLen;
    $UTMLink=mysql_connect(SQL_HOST,SQL_USER,SQL_PASSWORD,SQL_DB) or die(mysql_error());
    ...
    while ($desk[$x][$y-$step][0]) {
        $lastLetter = \Lang\Ru::$bukvy[( $desk[$x][$y-$step][1] < 999 ? $desk[$x][$y-$step][1] : $desk[$x][$y-$step][1] - 999 - 1)][0] . $lastLetter;
        $maxWordLen++;
        $step++;
    }
    ...
    if(mysql_num_rows($res=mysql_query($zapros))) {
        while($row=mysql_fetch_assoc($res))
            if (!isset($slovaPlayed[$row['slovo'] = mb_strtolower($row['slovo'], 'UTF-8')]) && checkWordFishki($fishki,$row['slovo'],$lastLetter,$lettersZvezd)) {
                ...
                for ($k=0;$k<$slovoNach;$k++) {
                    ...
                    $cells[$x][$yLastLetter-$k-1][1] = \Lang\Ru::getLetterCode($letter = mb_substr($row['slovo'],$slovoNach - 1 - $k,1,'UTF-8'));
                ...
                for ($k=0;$k<=mb_strlen($row['slovo'],'UTF-8') - $slovoNach - $lastLetterLen - 1;$k++) {
                    $cells[$x][$y+$k][0] = true;
                    $cells[$x][$y+$k][1] = \Lang\Ru::getLetterCode($letter = mb_substr($row['slovo'],$slovoNach + $lastLetterLen + $k,1,'UTF-8'));
                    if (isset($lettersZvezd[$letter])) {
                        $cells[$x][$yLastLetter-$k-1][2] = $lettersZvezd[$letter];
                        //Указали на занятую звездочку
                        $cells[$x][$y+$k][1] += (999 + 1);
