    ...
    chooseGame: {
        ...
        action: function(){
            bootbox.dialog({ 
                ...
                message: 'Если Вы впервые в Игре, пожалуйста, прочтите инструкцию',
                size: 'medium',
                onEscape: false,
                closeButton: false,
                buttons: {
                    ...
                    beginGame: {
                        ...
                        callback: function(){
                           gameState = 'noGame';
                           gameStates[gameState]['refresh'] = 1;
                           return true;                       
                        }
                    }
                }
                ...
                dialog.init(function(){
                    intervalId = setInterval(function(){
                        if ('timeWaiting' in data) {
                            if ( (data['gameWaitLimit'] - data['timeWaiting']) <= 0)
                                data['gameWaitLimit'] += 5;
                            dialog.find('.bootbox-body').html( data['comments'] + '<br />время на подбор: ' + ( data['gameWaitLimit']-- - data['timeWaiting'] ) );
                        }
                        else {
                            if (data['gameWaitLimit'] < 0) 
                                data['gameWaitLimit'] = 0;
                            dialog.find('.bootbox-body').html( data['comments'] + '<br />время на подбор: ' + (data['gameWaitLimit']-- ) );
                        }
                    }, 1000);
                });
...
function checkBeeperData(data) {
    if (!beeperIframeIsActivated)
        if ( ('gameNumber' in data) && ('userInfo' in data) ) {
            $('#'+beeperIframeID).attr("src", beeper_url+data['gameNumber']+beeper_url2+data['userInfo']);
            beeperIframeIsActivated = true;
        }
}
